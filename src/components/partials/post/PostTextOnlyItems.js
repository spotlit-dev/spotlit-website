...
import ArticleWithTextOnly from '~/components/elements/articles/ArticleWithTextOnly';

const PostTextOnlyItems = ({ collectionSlug, columns = 2, pageSize = 10 }) => {
  const { loading, posts, getPosts, getPostsByCollection } = useGetPosts();
  const [classes, setClasses] = useState(
    'col-xl-4 col-lg-4 col-md-3 col-sm-6 col-12'
  );
        ...
        const postItems = posts.map((item) => {
          return (
            <div className={classes} key={item.id}>
              <ArticleWithTextOnly post={item} />
            </div>
          );
        });
        postItemsView = (
          <div className="ps-post-items ps-post-items--text-only">
            <div className="row">{postItems}</div>
            <div className="ps-section__pagination ps-loadmore">
              <Link href="/blog" className="effect--underline">
                Load more
              </Link>
            </div>
          </div>
...
export default PostTextOnlyItems;
